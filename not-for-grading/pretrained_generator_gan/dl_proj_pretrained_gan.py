...
IMAGE_SIZE = 256
# EPOCHS = 1
EPOCHS = 30  # More epochs for GAN training
...
def gan_loss(predictions, target_is_real, device):
    ...
    if target_is_real:
        ...
        target = torch.ones_like(predictions) * (1.0 - LABEL_SMOOTHING)
    else:
        # Fake labels: 0.0
...
def run_inference(generator, data_folder, output_folder="gan_inference", num_images=50):
    """
    Run inference on first 50 images after training
    """
    ...
    # Random sample
    indices = torch.arange(min(num_images, len(full_dataset)))
...
def train_gan(generator, data_folder, pretrained_path=None, save_interval=5):
    ...
    # ---------- NEW: track best generator ----------
    best_g_loss = float('inf')
    best_g_path = "best_gan_generator.pth"
    best_d_path = "best_gan_discriminator.pth"
    ...
    for epoch in range(EPOCHS):
        ...
        print(f"\n[Epoch {epoch + 1}] "
              ...
              f"GAN_loss: {avg_gan_loss:.4f}")

        # Save ONE sample image per epoch
        save_sample_images(generator, val_loader, epoch + 1)

        # ---------- NEW: save best model so far ----------
        if avg_g_loss < best_g_loss:
            best_g_loss = avg_g_loss
            torch.save(generator.state_dict(), best_g_path)
            torch.save(discriminator.state_dict(), best_d_path)
            print(f"✓ New best generator (G_loss={best_g_loss:.4f}) saved to {best_g_path}")

        # # Save checkpoints every few epochs (as before)
...
def train_colorization_model(
    ...
    if inference_count is not None:
        print(f"Using first {inference_count} images for inference...")
